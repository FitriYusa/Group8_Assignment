    ...
    while (completed != arrivalArr.length) {
      ...
      if (burst_remaining[idx] - Number(tq) > 0) {
        ...
        if (startGantt >= Number(corres[idx].start_time)) {
          ...
          // This is a 2D array that stores the values corresponding to each process currently in the ready queue.
          // Each row in the array represents an index of a process.
          // The structure of each row is as follows: ganttChartData[index] = [processIndex, processId, startTime, endTime]
        }
        startGantt = current_time;
      } else {
        ...
        if (startGantt >= Number(corres[idx].start_time)) {
          ...
          // This 2D array holds the information for each process currently in the ready queue.
          // Each row represents an index of a process, and it contains four columns of data.
          // The structure of each row is as follows: ganttChartData[index] = [index, processId, startTime, endTime]
          // Index: The index of the process in the ready queue.
          // ProcessId: The unique identifier of the process.
          // StartTime: The time at which the process starts.
          // EndTime: The time at which the process ends.
        }
        startGantt = current_time;
